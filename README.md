...
| 方法 | 说明 | 示例 |
|------|------|------|
| `zxd.busCreate(options)` | 创建独立总线实例 | `var bus = zxd.busCreate()` |
| `zxd.busOn(topic, fn)` | 监听默认总线事件 | `zxd.busOn('user:login', fn)` |
| `zxd.busOnce(topic, fn)` | 只监听一次 | `zxd.busOnce('ready', fn)` |
| `zxd.busEmit(topic, payload, meta)` | 发送事件 | `zxd.busEmit('cart:add', {id:1})` |
| `zxd.busWait(topic, timeout)` | Promise 等待事件 | `await zxd.busWait('done', 3000)` |
| `zxd.busHistory(topic?)` | 查看历史事件 | `zxd.busHistory('cart:add')` |

```javascript
// 创建独立事件总线（推荐用于模块隔离）
const bus = zxd.busCreate({ debug: true, replayLimit: 100 });

const un = bus.on('order:create', (payload) => {
  console.log('收到订单', payload.orderId);
});

bus.emit('order:create', { orderId: 'A10086' });
un(); // 取消监听
```
...
| 方法 | 说明 | 示例 |
|------|------|------|
| `zxd.createStore(options)` | 创建 Store | `var store = zxd.createStore({...})` |
| `store.getState(path?)` | 读取状态 | `store.getState('user.name')` |
| `store.setState(path, value)` | 设置状态 | `store.setState('counter', 10)` |
| `store.commit(type, payload)` | 执行 mutation | `store.commit('inc', 2)` |
| `store.dispatch(type, payload)` | 执行 action(支持异步) | `await store.dispatch('loadUser')` |
| `store.watch(path, handler)` | 监听状态变化 | `store.watch('cart', fn)` |
| `store.module.register(name, def)` | 注册子模块 | `store.module.register('todo', {...})` |
| `store.persistSave/load()` | 手动持久化/恢复 | `store.persistSave()` |

```javascript
const store = zxd.createStore({
  name: 'app',
  strict: true,
  ...
  actions: {
    async login({ commit }, payload) {
      // 模拟请求
      await zxd.wait(300);
      commit('setUser', { name: payload.name });
      return true;
    }
  },
  persist: { enabled: true, mode: 'local', key: 'zx_app_store' }
});

store.watch('count', (next, prev) => {
  console.log('count变化', prev, '->', next);
}, { immediate: true });
```

---

### 3) 路由系统 `routerx`（Hash 模式）

| 方法 | 说明 | 示例 |
|------|------|------|
| `zxd.createRouter(options)` | 创建路由器 | `var router = zxd.createRouter({...})` |
| `router.start()` | 启动路由监听 | `router.start()` |
| `router.push(path, query?)` | 跳转 | `router.push('/user/1', {tab:'base'})` |
| `router.replace(path, query?)` | 替换当前路由 | `router.replace('/login')` |
| `router.beforeEach(fn)` | 全局前置守卫 | `router.beforeEach((to,from,next)=>{})` |
| `router.afterEach(fn)` | 全局后置钩子 | `router.afterEach((to,from)=>{})` |
| `router.addRoute(route)` | 动态添加路由 | `router.addRoute({path:'/x',component:'x'})` |
| `router.current()` | 获取当前匹配 | `router.current()` |

```javascript
const router = zxd.createRouter({
  view: '#app',
  routes: [
    { path: '/', component: () => '<h2>首页</h2>' },
    { path: '/users/:id', component: (to) => `<h2>用户 ${to.params.id}</h2>` }
  ],
...
router.beforeEach((to, from, next) => {
  // 例如未登录时拦截
  if (to.path === '/admin' && !localStorage.getItem('token')) return '/login';
  next(true);
});
...
| 方法 | 说明 | 示例 |
|------|------|------|
| `zxd.httpCreate(options)` | 创建客户端实例 | `const api = zxd.httpCreate({...})` |
| `api.request(config)` | 通用请求 | `api.request({url:'/x',method:'POST'})` |
| `api.get/post/put/patch/del` | 常用请求方法 | `api.get('/users')` |
| `api.upload(url,file,field,extra)` | 文件上传 | `api.upload('/upload', file)` |
| `api.useRequest(fn)` | 请求拦截器 | `api.useRequest(cfg=>cfg)` |
| `api.useResponse(fn)` | 响应拦截器 | `api.useResponse(res=>res)` |
| `api.useError(fn)` | 错误拦截器 | `api.useError((e,cfg)=>e)` |

```javascript
const api = zxd.httpCreate({
  baseURL: 'https://jsonplaceholder.typicode.com',
  timeout: 8000,
  retries: 2,
  retryDelay: 300,
  headers: { 'X-App': 'zxlite' }
});

api.useRequest((cfg) => {
  cfg.headers.Authorization = 'Bearer demo-token';
  return cfg;
});
...
const list = await api.get('/posts', { _limit: 5 });
...
| 方法 | 说明 | 示例 |
|------|------|------|
| `zxd.cacheCreate(options)` | 创建缓存实例 | `const cache = zxd.cacheCreate({...})` |
| `cache.set/get/has/remove` | 基础增删查 | `cache.set('k', v, 1000)` |
| `cache.touch(key, ttl?)` | 刷新 TTL | `cache.touch('profile', 5000)` |
| `cache.keys()` | 获取 key 列表 | `cache.keys()` |
| `cache.clean()` | 清理过期数据 | `cache.clean()` |
| `cache.stats()` | 获取统计 | `cache.stats()` |

默认还提供了快捷方法：
`zxd.cacheSet / zxd.cacheGet / zxd.cacheHas / zxd.cacheDel / zxd.cacheClear`
...
| 方法 | 说明 | 示例 |
|------|------|------|
| `zxd.queueCreate(options)` | 创建队列 | `const q = zxd.queueCreate({concurrency:3})` |
| `q.add(task, options)` | 添加任务 | `q.add(() => fetch(...))` |
| `q.pause()/resume()` | 暂停/恢复 | `q.pause()` |
| `q.cancel(id)` | 取消等待中的任务 | `q.cancel('task_xxx')` |
| `q.idle()` | 等待队列清空 | `await q.idle()` |
| `q.snapshot()` | 当前快照 | `q.snapshot()` |
| `q.stats()` | 统计信息 | `q.stats()` |

```javascript
const q = zxd.queueCreate({ concurrency: 2, retry: 1, retryDelay: 200 });
...
| 方法 | 说明 | 示例 |
|------|------|------|
| `zxd.createValidator(schema, options)` | 创建校验器 | `const v = zxd.createValidator({...})` |
| `v.validate(data)` | 同步校验 | `const r = v.validate(form)` |
| `v.validateAsync(data)` | 异步校验 | `await v.validateAsync(form)` |
| `v.field(name, data)` | 只校验某个字段 | `v.field('email', form)` |
| `v.extendRule(name, fn)` | 注册自定义规则 | `v.extendRule('phone', fn)` |

支持规则：
`required / type / min / max / minLen / maxLen / len / pattern / email / url / enum / sameAs / custom / async`

```javascript
const validator = zxd.createValidator({
  username: [
    'required',
    { rule: 'minLen', value: 3, message: '用户名至少3位' }
  ],
  email: ['required', 'email'],
  password: [{ rule: 'minLen', value: 6 }],
  confirmPassword: [{ rule: 'sameAs', value: 'password', message: '两次密码不一致' }],
...
| 方法 | 说明 | 示例 |
|------|------|------|
| `zxd.tplx.render(template, data, options)` | 模板渲染 | `zxd.tplx.render('Hi {{name}}', {name:'Tom'})` |
| `zxd.tplx.compile(template)` | 编译模板 | `const render = zxd.tplx.compile(...)` |
| `zxd.tplx.list(items, template)` | 列表渲染 | `zxd.tplx.list(arr, '<li>{{name}}</li>')` |
| `zxd.tplx.mount(el, template, state)` | 挂载模板并可更新 | `const vm = zxd.tplx.mount(...)` |
| `zxd.tplx.define(name, def)` | 定义组件 | `zxd.tplx.define('Card', {...})` |
| `zxd.tplx.create(name, props)` | 创建组件实例 | `const c = zxd.tplx.create('Card', {...})` |

```javascript
const html = zxd.tplx.render('<h3>{{title}}</h3><p>{{desc}}</p>', {
  title: 'zxlite',
  desc: '模板渲染示例'
});
document.getElementById('demo').innerHTML = html;

const vm = zxd.tplx.mount('#demo2', '<p>count = {{count}}</p>', { count: 1 });
vm.patch({ count: 2 });
```
...
| 方法 | 说明 | 示例 |
|------|------|------|
| `zxd.datex.parse(input)` | 解析日期 | `zxd.datex.parse('2026-01-01')` |
| `zxd.datex.format(input, pattern)` | 格式化 | `zxd.datex.format(Date.now(), 'YYYY/MM/DD')` |
| `zxd.datex.add(input, amount, unit)` | 日期加减 | `zxd.datex.add(new Date(), 7, 'day')` |
| `zxd.datex.diff(a, b, unit)` | 时间差 | `zxd.datex.diff(a, b, 'day')` |
| `zxd.datex.startOf/endOf` | 取开始/结束时间 | `zxd.datex.startOf(new Date(),'month')` |
...
| 方法 | 说明 | 示例 |
|------|------|------|
| `zxd.pipeline(tasks, input)` | 串行流水线 | `await zxd.pipeline([fn1,fn2], init)` |
| `zxd.parallel(tasks, limit)` | 并发执行任务 | `await zxd.parallel(tasks, 3)` |
| `zxd.retry(fn, options)` | 重试封装 | `await zxd.retry(fetchFn, {times:3})` |
| `zxd.escape/unescape` | HTML 转义反转义 | `zxd.escape('<h1>')` |
| `zxd.ue/ud` | URL 编解码 | `zxd.ue('你好')` |
