...
(function(global) {
    ...
    Z.importfs = async function() {
        ...
        for (var i = 0; i < args.length; i++) {
            promises.push(Z.importf(args[i]));
        }
    ...
    Z.importfa = async function(folderPath, alias) {
        var success = await Z.importf(folderPath);
        if (success && alias) {
            var pkgName = folderPath.replace(/\/$/, '').split('/').pop();
            return Z.alias(pkgName, alias);
        }
        return success;
    };
