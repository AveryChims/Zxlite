...
(function(global) {
    ...
    function createDialogCore(opts) {
        ...
        // 获取当前主题的文本颜色
        var theme = Z._themes[Z._currentTheme] || Z._themes.light1;
        var dialogTextColor = theme.text;
        var dialogTextSecColor = theme.textSec;
        ...
        dialog.style.cssText = 'background:' + s.bg + ';border-radius:' + s.radius + ';padding:' + s.padding + ';min-width:280px;max-width:90%;box-shadow:' + s.shadow + ';color:' + dialogTextColor + ';border:1px solid ' + s.border + ';';
        ...
        if (customHtml) {
            ...
            titleEl.style.cssText = 'margin:0 0 12px 0;font-size:18px;font-weight:600;color:' + dialogTextColor + ';';
            ...
            if (isInput) {
                ...
                inputEl.style.cssText = 'width:100%;padding:10px;margin:0 0 20px 0;border:1px solid ' + s.border + ';border-radius:' + s.btnRadius + ';background:' + s.bg + ';color:' + dialogTextColor + ';';
                dialog.appendChild(inputEl);
            } else if (content) {
                var contentEl = document.createElement('p');
                contentEl.textContent = content;
                contentEl.style.cssText = 'margin:0 0 24px 0;color:' + dialogTextSecColor + ';line-height:1.5;font-size:14px;';
    ...
    Z.dialog = function(title, content, btn1, btn2, cb1, cb2) {
        ...
        if (arguments.length >= 5 && typeof btn2 === 'function') {
            btns.push({ text: btn1, cb: cb1 });
            btns.push({ text: btn2, cb: cb2 });
        } else if (arguments.length >= 3) {
            btns.push({ text: btn1, cb: btn2 || cb1 });
        } else {
            btns.push({ text: '确定', cb: null });
        }
        return createDialogCore({ title: title, content: content, btns: btns, outsideClose: false });
    };
    ...
    Z.style = function(theme, color) {
        ...
        if (color && /^#([0-9A-F]{3}){1,2}$/i.test(color)) {
            ...
            styleEl.textContent = `
                ...
                h1, h2, h3, h4, h5, h6, p, a, i, b, strong, em, span, li, label { color: var(--zx-text) !important; }
                ...
                input:focus, textarea:focus, select:focus { outline: none; border-color: var(--zx-primary) !important; box-shadow: 0 0 0 2px var(--zx-primary-light); }
                .api-desc { background: var(--zx-primary-light) !important; color: var(--zx-text) !important; }
...
if (typeof console !== 'undefined') console.log('zxlite.js v1.3.7 loaded - 优化深色模式Toast阴影、颜色协调');